...
def plot8(data: pd.DataFrame, demarkation=[], customeOffset=None):
    # if args.drop_start:
    ...
    if customeOffset is not None:
        headingShift = float(customeOffset)
        print(np.rad2deg(headingShift))
        print(np.rad2deg(np.mean(data.psi)))
        data.psi = np.rad2deg(data.psi - np.mean(data.psi))
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--demarcate', help='Marks maneuver sets (Requires --marker_path)',
                        action='store_true', default=False)
    parser.add_argument('--custom-offset')
    ...
    if args.marker_path:
        ...
        data = data.reset_index(drop=True)
    plot8(data, demarcations, args.custom_offset)
